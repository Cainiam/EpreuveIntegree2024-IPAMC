<div class="container mx-auto py-8">
    ...
    <div class="bg-blue-50 shadow-md rounded-lg p-4">
        <h2 class="text-lg font-semibold mb-2">Appuyez pour payer avec PayPal :</h2>
        <form action="{{ route('paypal_payment') }}" method="POST">
            @csrf
            <input type="hidden" name="shipping_id" value="{{ $selectedShipping }}">
            <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg" {{ !$selectedShipping ? 'disabled' : '' }}>Payement PayPal</button>
        </form>
    </div>
</div>