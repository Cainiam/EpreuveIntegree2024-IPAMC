...
class FigureEdit extends Component
{
    use WithFileUploads;
    public $figureId, $tvas, $brands, $scales, $categories;
    ...
    public function mount($id)
    {
        ...
        $this->description = $figure->description;
        $this->existing_path = $figure->image_path;
        $this->price = $figure->price;
        $this->tva_id = $figure->tva_id;
        $this->brand_id = $figure->brand_id;
        $this->collection = $figure->collection;
        $this->character_name = $figure->character_name;
        $this->series_title = $figure->series_title;
        $this->sculptor_name = $figure->sculptor_name;
        $this->material = $figure->material;
        $this->height = $figure->height;
        $this->scale_id = $figure->scale_id;
        $this->release_date = $figure->release_date;
        $this->stock_qty = $figure->stock_qty;
        $this->reference = $figure->reference;
        $this->ean = $figure->ean;
        $this->state = $figure->state;
        $this->category_id = $figure->category_id;
        $this->isVisible = $figure->isVisible;
    }

    protected function rules()
    {
        return [
            'name' => ['required', 'string', 'min:2', 'max:255', Rule::unique('figures')->ignore($this->figureId)],
    ...
    public function updateFigure()
    {
        ...
        $figure = Figure::findOrFail($this->figureId);
        ...
        //$img = $this->image_path ? $this->image_path->store('figures', 'public') : $this->figure->image_path;
        ...
        session()->flash('message', 'Figurine mise à jour avec succès.');
        return redirect()->route('gerantdashboard');
    }

    public function render()
    {
        return view('livewire.gerant.figure-edit')->layout('layouts.app');
    }
}
